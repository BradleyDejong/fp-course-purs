...
functorSpec = do
  ...
  describe "(<$)"
    $ do
        ...
        it "Always maps a constant value over List"
          $ quickCheck \(x :: Int) a b c -> (x <$ ((a :. b :. c :. Nil) :: List Int)) == (x :. x :. x :. Nil)
        it "Always maps a constant value over Full (Optional)"
          $ quickCheck \(x :: Int) (q :: Int) -> (x <$ Full q) == Full x
  describe "void"
    $ do
