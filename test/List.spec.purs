...
import Prelude
import Course.List (List(..), filter, flatMap, flatten, flattenAgain, foldLeft, headOr, hlist, infinity, largeList, length, listh, product, sum, take, (:.), seqOptional, find, lengthGT4, reverse, produceN)
...
listSpec = do
  ...
  describe "filter"
    $ do
        ...
        it "filter (const True) is identity (headOr)"
          $ quickCheck \x -> headOr x (filter (const true) infinity) == 0 <?> "Test failed for input " <> show x
        it "filter (const True) is identity"
          $ quickCheck \x -> filter (const true) (x :: List Int) === x
        it "filter (const False) is the empty list"
          $ quickCheck \x -> filter (const false) (x :: List Int) == Nil <?> "Test failed for input " <> show x
  describe "map or <$>"
    $ do
        it "add 10 on list"
          $ ((+) 10 <$> 1 :. 2 :. 3 :. Nil) `shouldEqual` (11 :. 12 :. 13 :. Nil)
        it "headOr after map"
          $ quickCheck \(x :: Int) -> headOr x ((+) 1 <$> infinity) == 1
        it "map id is id"
          $ quickCheck \x -> (\y -> y) <$> x === (x :: List Int)
  describe "(<>)"
    $ do
        it "(1..6)"
          $ ((1 :. 2 :. 3 :. Nil) <> (4 :. 5 :. 6 :. Nil)) `shouldEqual` listh [ 1, 2, 3, 4, 5, 6 ]
        it "append empty to infinite"
          $ quickCheck \x -> headOr x (Nil <> infinity) == 0
        it "append anything to infinity"
          $ quickCheck \x y -> headOr x (y <> infinity) == headOr 0 y
        it "associativity"
          $ quickCheck \x y (z :: List Int) -> (x <> y) <> z == x <> (y <> z)
        it "append to empty list"
          $ quickCheck \(x :: List Int) -> x <> Nil == x
  describe "flatten"
    $ do
        it "(1..9)"
          $ flatten ((1 :. 2 :. 3 :. Nil) :. (4 :. 5 :. 6 :. Nil) :. (7 :. 8 :. 9 :. Nil) :. Nil) `shouldEqual` listh [ 1, 2, 3, 4, 5, 6, 7, 8, 9 ]
        it "flatten (infinity :. y)"
          $ quickCheck \x y -> headOr x (flatten (infinity :. y :. Nil)) == 0
        it "flatten (y :. infinity)"
          $ quickCheck \x y -> headOr x (flatten (y :. infinity :. Nil)) == headOr 0 y
        it "sum of lengths == length of flattened"
          $ quickCheck \(x :: List (List Int)) -> sum (map length x) == length (flatten x) <?> "Test failed for input " <> show x
  describe "flatMap"
    $ do
        it "lists of Integer"
          $ flatMap (\x -> x :. x + 1 :. x + 2 :. Nil) (1 :. 2 :. 3 :. Nil) `shouldEqual` listh [ 1, 2, 3, 2, 3, 4, 3, 4, 5 ]
        it "flatMap id flattens a list of lists"
          $ quickCheck \x y -> headOr x (flatMap identity (infinity :. y :. Nil)) == 0
        it "flatMap id on a list of lists take 2"
          $ quickCheck \x y -> headOr x (flatMap identity (y :. infinity :. Nil)) == headOr 0 y
        it "flatMap id == flatten"
          $ quickCheck \(x :: List (List Int)) -> flatMap identity x == flatten x
  describe "flattenAgain"
    $ do
        it "lists of Integer"
          $ quickCheck \(x :: List (List Int)) -> flatten x == flattenAgain x
  describe "seqOptional"
    $ do
        ...
        it "empty list"
          $ let
              empty = Nil :: List (Optional Int)
            in
              seqOptional empty `shouldEqual` Full Nil
        it "contains Empty"
  ...
  describe "reverse"
    $ do
        ...
        it "reverse then append is same as append then reverse"
          $ quickCheck \x (y :: List Int) ->
  ...
  describe "produce"
    $ do
        it "increment"
          $ produceN 4 ((+) 1) 0 `shouldEqual` (0 :. 1 :. 2 :. 3 :. Nil)
        it "double"
          $ produceN 4 ((*) 2) 1 `shouldEqual` (1 :. 2 :. 4 :. 8 :. Nil)
